...
fun dates_in_months (dateList : (int*int*int) list, months : int list) =
  if null months
  then []
  else let fun add_two_list (list1 : (int*int*int) list, list2 : (int*int*int) list) =
	     if null list1
	     then list2
	     else (hd list1) :: add_two_list(tl list1, list2)
       in
	   add_two_list(dates_in_month(dateList, hd months), dates_in_months(dateList, tl months))
       end
	   


(* function 6 : get_nth Write a function get_nth that takes a list of
strings and an int n and returns the n th element of the list where
the head of the list is 1st. Do not worry about the case where the
list has too few elements: your function may apply hd or tl to the
...
(* function 7 : date_to_string Write a function date_to_string that
takes a date and returns a string of the form January 20, 2013 (for
example). Use the operator ^ for concatenating strings and the library
function Int.toString for converting an int to a string. For producing
the month part, do not use a bunch of conditionals.  Instead, use a
list holding 12 strings and your answer to the previous problem. For
consistency, put a comma following the day and use capitalized English
month names: January, February, March, April, May, June, July, August,
September, October, November, December.*)
							   
fun date_to_string (date : (int*int*int)) =
  let val monthsList = ["January",
		    "February",
		    "March",
		    "April",
		    "May",
...
fun number_before_reaching_sum (sum : int, aList : int list) =
  if null aList orelse (hd aList) >= sum
  then 0
  else
      number_before_reaching_sum (sum - (hd aList), (tl aList)) + 1
...
fun what_month (day : int) =
  let val daysList = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
  in
      number_before_reaching_sum (day, daysList) + 1
  end
...
evaluates to NONE if the list has no dates and SOME d if the date d is the oldest date in the list. *)
									     ...
									     fun oldest (datesList : (int*int*int) list) =
										   ...
										   let val currentMax =  oldest(tl datesList)
										   in if isSome currentMax andalso is_older(valOf currentMax, hd datesList)
										      then currentMax
										      else SOME (hd datesList)
										   end
